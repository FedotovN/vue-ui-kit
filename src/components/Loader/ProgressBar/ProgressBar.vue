<script setup lang="ts">
    ...
    const clampedValue = computed(() => props.value > props.max ? props.max : props.value < props.min ? props.min : props.value);
    const percentage = computed(() => ((clampedValue.value / props.max) * 100).toFixed(0));
    const styles = computed(() => {
        const [red, blue, green] = getColor(props.color);
        return {
...
<template>
    <div class="progress-bar-wrapper" :style="styles">
        <small class="progress-bar-wrapper-label">
            <small v-if="showProgress && !$slots.default">
                {{  percentage  }}%
            </small>
