...
const items = computed(() => {
  return retrievedVNodes.value.filter(
    (node) =>
      (node as unknown as ComponentInternalInstance).type.__name ===
      "BaseTabItem",
  ) as tabVNode[];
});
const defaultValue = computed(() => {
  ...
  if (props.defaultTabId)
    return items.value.find((item) => item.props.id == props.defaultTabId);
});
