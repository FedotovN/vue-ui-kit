...
<template>
  <div
    :style="style"
    class="base-input"
    :class="{ disabled, error: !!errorMessage }"
  >
    <label
      v-if="label"
      :for="inputId"
    ...
    <div class="base-input__body">
      <input
        ...
        @focusin="onFocusIn($event.target as HTMLInputElement)"
        @focusout="onFocusOut($event.target as HTMLInputElement)"
        v-model="localValue"
        class="base-input__input"
        :id="inputId"
        :placeholder="toShowPlaceholder.toString()"
    ...
    <label
      ...
      :class="{ wrap: errorWrap, focused: isFocused }"
      v-if="!!errorMessage"
      >{{ errorMessage }}</label
    >
  </div>
