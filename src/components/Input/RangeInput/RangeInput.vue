<script setup lang="ts">
  ...
  function onInput(e: Event, side: 'min' | 'max'){
    ...
    input.value = (normalize(minValue?.value!, maxValue?.value!, value, side, minimalRange.value, limit.value)).toString();
  ...
  <div class="range-input-wrapper" :style="style">
    <div class="range-input-body">
      <div class="range-input-path">
        ...
        <input type="range" class="range-input" :value="props.minValue" @input="e => onInput(e, 'min')" :step="step" :max="limit" :min="minimum">
        <input type="range" class="range-input" :value="props.maxValue" @input="e => onInput(e, 'max')" :step="step" :max="limit" :min="minimum">
    </div>
  </div>
</template> 
