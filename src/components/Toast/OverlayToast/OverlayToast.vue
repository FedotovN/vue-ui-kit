<script setup lang="ts">
    ...
    const props = withDefaults(defineProps<OverlayToastProps>(), {
...
<template>
    <Teleport to="body">
        <div class="overlay-toast-wrapper">
            <div class="overlay-toast-container">
                <TransitionGroup name="toast-notification-transition">
                    <toast-notification v-for="toast in toasts" :toast="toast" :key="toast.id" @click="remove(toast.id)" :max-width="maxWidth" :min-width="minWidth"></toast-notification>
                </TransitionGroup>
            </div>
        </div>
    </Teleport>
