...
const columns = getVNodesOfComponent(useSlots().default(), BaseTableColumn).map(
  (cols) => cols.props as BaseTableItemProps,
);
...
const currElements = computed(() => {
  return props.items.slice(
    currPage.value * +props.itemsPerPage,
    (currPage.value + 1) * +props.itemsPerPage,
  );
});
  ...
  <div class="base-table-wrapper" :style="style">
    ...
    <div class="base-table-rows" v-if="currElements.length">
      <div
        ...
        :class="{ clickable: !!itemClickCallback }"
        v-for="item in currElements"
        @click="() => itemClickCallback?.(item)"
      >
        <div class="base-table-item-col" v-for="col in columns">
          <small>{{
            getColumnValue(
              item,
              col.field,
              col["valueCallback"] || col["value-callback"],
            )
          }}</small>
        </div>
    ...
    <div class="base-table-fallback-content" v-else>Nothing here yet.</div>
    <div class="base-table-footer" v-if="showFooter">
      <div class="base-table-pagination-bar" v-if="showPagination">
        <small class="pagination-bar-count"
          >{{ currPage * +itemsPerPage + 1 }} -
          {{
            +itemsPerPage * (currPage + 1) -
            (+itemsPerPage - currElements.length)
          }}</small
        >
        <div class="pagination-bar-pages-wrapper">
          <div
            class="pagination-bar-pages"
            :style="{
              left:
                -(
                  32 * +movePaginationBarEach +
                  4.5 * (+movePaginationBarEach - 1)
                ) *
                  Math.floor(currPage / +movePaginationBarEach) +
                'px',
            }"
          >
            <div
              class="pagination-bar-page-button"
              :class="{ current: currPage + 1 === page }"
              v-for="page in pagesAmount + 1"
        ...
        <div
          ...
          v-if="currElements.length && showButtons"
        >
          <base-button
            :color="color"
            v-if="currPage - 1 > -1"
