...
<template>
  ...
  >
    ...
    <div
      class="base-file-badge-download"
      v-tooltip="`Download ${value.name || ''}`"
      v-if="toDownload && allowDownload"
      @click="downloadFile"
    >
      <svg
        width="36px"
    ...
    <div
      class="base-file-badge-delete"
      v-tooltip="`Delete ${value.name || ''}`"
      v-if="allowDelete"
      @click="$emit('delete')"
    >
      <svg
